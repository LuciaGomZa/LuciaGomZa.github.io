...
initPhysicWorld(); // Inicializacion mundo fisico: objetos físicos que no vemos 
initVisualWorld();
loadWorld();
render();
...
function esfera( radio, posicion, material ){
	...
	// Conectamos la forma visual (lo que veremos) con el objeto fisico que atiende a las fuerzas
}
...
function initPhysicWorld()
{
    ...
    world.addMaterial( materialEsfera ); // Añadimos materiales al mundo fisico
    ...
    var sphereGroundContactMaterial = new CANNON.ContactMaterial(groundMaterial,materialEsfera,
    										    				{ friction: 0.3, 
																	restitution: 0.7 });
	// Entre el material del suelo y de la esfera se establecen caract fisicas de rozamiento y rebote
    world.addContactMaterial(sphereGroundContactMaterial);

    // Suelo (se le ha aisnado objeto visual)
    var groundShape = new CANNON.Plane();
    var ground = new CANNON.Body({ mass: 0, material: groundMaterial }); // mass=0: no le afectan las fuerzas, se queda fijo
    ground.addShape(groundShape); // Se le asigna la forma
    ground.quaternion.setFromAxisAngle(new CANNON.Vec3(1,0,0),-Math.PI/2); // quaternion son para asignar rotaciones (angulo, eje)
    world.addBody(ground);

    // Paredes (no objeto visual)
...
{
	// (es el mundo de siempre, el que haciamos antes)
...
function loadWorld()
{
	// Genera las esferas (para cada una necesitamos un material del que esté construida)
	var materialEsfera;
	for( i=0; i<world.materials.length; i++){
		if( world.materials[i].name === "sphereMaterial" ) materialEsfera = world.materials[i];
	}
	// Creamos las n esferas
	for (var i = 0; i < nesferas; i++) {;
		var e = new esfera( 1/2, new CANNON.Vec3( -1, i+1, 0 ), materialEsfera );
		world.addBody( e.body ); // OJO, añadimos cada parte donde toca
		scene.add( e.visual );
		esferas.push( e );
	};
    // Restricciones (para conectar las esferas contiguas)
 	for (var i = 0; i < esferas.length-1; i++) {
    	var restriccion = new CANNON.PointToPointConstraint(esferas[i].body,
    												    new CANNON.Vec3( 0, 1/2, 0), // punto de la esfera i se une
    												    esferas[i+1].body,
														new CANNON.Vec3( 0, -1/2, 0) ); // el punto de la esfera i+1
...
function update()
{
	...
	// Se resuelve el sistema de ecuaciones en donde se introducen las fuerzas que actuan 
	// en cada momento indicado por el reloj. Se recalculan las posiciones en consecuencia. 
